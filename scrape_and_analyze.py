#!/usr/bin/env python3
"""
Renegade AI 文献监控脚本 v4.0 — 多模型复证版
功能：
1. 多源抓取：arXiv + Semantic Scholar
2. 增量去重，避免重复分析
3. 预筛选过滤无关论文
4. 多模型并发分析（通过 OpenRouter + DeepSeek 直连）
5. 生成多模型对比报告
"""
...
# 实际分析模型清单（可自由组合）
ANALYSIS_MODELS = FREE_MODELS + CHEAP_PAID_MODELS  # 初始建议只用免费+便宜付费
...
def analyze_single_model(paper, model_name, client, api_source="openrouter"):
...
def analyze_paper_multi_model(paper, models):
    ...
    # DeepSeek 直连
    if deepseek_client:
        tasks.append(("deepseek/deepseek-chat", deepseek_client, "deepseek"))
    # OpenRouter 模型
    if openrouter_client:
        for model_name in models:
            tasks.append((model_name, openrouter_client, "openrouter"))

    results = []
    with ThreadPoolExecutor(max_workers=min(len(tasks), 8)) as executor:
        future_map = {
            executor.submit(analyze_single_model, paper, model_name, client, source): model_name
            for model_name, client, source in tasks
        }
    ...
    if not results:
        merged = {
            "relevance": 0, "summary_cn": "所有模型调用失败",
            "implications": "N/A", "chapter_target": "",
            "update_type": "", "urgency": "background", "action": "忽略"
        }
    else:
        best = max(results, key=lambda r: r.get("relevance", 0))
        merged = {k: v for k, v in best.items() if not k.startswith("_")}
...
def generate_markdown_multi(papers_data, keywords):
    ...
    if high:
        ...
        for i, d in enumerate(high, 1):
            ...
            # 多模型评分对比表
            if d.get("models_results"):
                lines.append(f"\n**🧠 多模型评分对比:**")
                lines.append("| 模型 | 相关度 | 更新类型 | 紧迫度 |")
                lines.append("|------|--------|----------|--------|")
                for r in sorted(d["models_results"], key=lambda x: x.get("relevance", 0), reverse=True):
                    model_short = r.get("_model", "?").split("/")[-1][:25]
                    lines.append(
                        f"| {model_short} | {r.get('relevance', '?')} | "
                        f"{r.get('update_type', '?')} | {r.get('urgency', '?')} |"
                    )
            lines.append("")
...
def main():
    print("=" * 50)
    print("🚀 Renegade AI 文献监控系统 v4.0 启动")
    print("=" * 50)
