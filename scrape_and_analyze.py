#!/usr/bin/env python3
"""
Renegade AI 文献监控脚本 v4.4.0 — 学术+资讯双雷达整合版
...
from news_sources import fetch_all_news      # 新增：资讯聚合模块
            ...
            try:
                ...
                for p in data:
                    ...
                    papers.append({
                        ...
                        "source": "semantic_scholar",   # 新增来源标记
                    })
                    seen_ids.add(pid)
                    count += 1
...
# 多模型分析
# ------------------------------------------------------------------
SYSTEM_PROMPT = """你是《Renegade AI: Catalyst for Human Cognitive Evolution》(v5.3) 的智能编辑助手。你需要判断一篇学术论文或一篇新闻报道是否与本书的核心论点相关，并给出具体的更新建议。
...
## 任务
阅读内容（可能是论文摘要或新闻报道），返回严格JSON（不要任何额外文字）。"""
...
阅读新闻摘要，返回严格 JSON：
{
    ...
    "case_value": "high / medium / low – 是否适合作为书中案例或类比？",
...
# ------------------------------------------------------------------
def main() -> None:
    logger.info("🚀 Renegade AI 文献+资讯监控系统 v4.4.0 启动")
    ...
    if getattr(Config, "ENABLE_NEWS_API", False) or getattr(Config, "ENABLE_RSS_FEEDS", False):
        ...
        logger.info("⏭️ 资讯源未启用（ENABLE_NEWS_API 和 ENABLE_RSS_FEEDS 均为 False）")

    # 合并所有条目（论文 + 资讯）
    all_papers = deduplicate_papers(arxiv_papers + s2_papers + news_articles)
    logger.info(f"📄 合并去重后共 {len(all_papers)} 篇（论文 + 资讯）")
