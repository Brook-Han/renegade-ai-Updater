...
# 新闻分析阈值
RELEVANCE_THRESHOLD = getattr(Config, "NEWS_RELEVANCE_THRESHOLD", 3)
...
def fetch_news_with_cache(keywords: list[str]) -> list[dict]:
    ...
    for item in raw_news:
        ...
        news_item = {
            ...
            "summary": safe_str(item.get("summary", item.get("description", ""))),
...
def prescreen_news(news_list: list[dict]) -> list[dict]:
    ...
    # 理论模型关键词（小写匹配）
    theory_terms = [
        "consensus cage", "renegade ai", "demand-side discipline",
        "capital", "rlhf", "alignment", "human-ai", "cognitive",
        "temporal sovereignty", "financialization", "token",
        "dark time", "evolutionary", "signal", "bias", "manipulation",
        "automation", "governance", "ethics", "trust", "persuasion",
    ]
    
    # 领域关键词
    domain_terms = [
        "ai", "artificial intelligence", "llm", "language model", "gpt",
        "machine learning", "algorithm", "automation", "chatbot",
        "tech policy", "regulation", "surveillance", "privacy",
        "labor", "employment", "economy", "market", "finance",
    ]
    ...
    for item in news_list:
        ...
        # 至少命中1个理论词 + 1个领域词
        has_theory = any(t in text for t in theory_lower)
        has_domain = any(t in text for t in domain_lower)
...
# 报告生成（纯资讯版）
# ------------------------------------------------------------------
def generate_news_report(news_data: list[dict], keywords: list[str]) -> str:
    """生成Markdown资讯报告"""
    today = datetime.date.today().isoformat()
    
    # 分类统计
    high = [d for d in news_data if d["analysis"].get("relevance", 0) >= 7 and d["analysis"].get("case_value") in CASE_VALUE_FILTER]
    medium = [d for d in news_data if 4 <= d["analysis"].get("relevance", 0) < 7]
    low = [d for d in news_data if d["analysis"].get("relevance", 0) < 4 or d["analysis"].get("action") == "忽略"]
    ...
    lines = [
        ...
        "---\n",
        "## 📊 快速概览\n",
        f"- 🔴 高价值 (≥7分 + 高案例价值): **{len(high)}**",
        f"- 🟡 中相关 (4-6.9分): **{len(medium)}**",
        f"- ⚪ 低相关/忽略: **{len(low)}**\n",
    ]
    
    # 🚨 紧急更新清单
    urgent = [d for d in news_data if d["analysis"].get("urgency") == "immediate" and d["analysis"].get("relevance", 0) >= 6]
    if urgent:
    ...
    report_path = OUTPUT_DIR / f"news_report_{now.date().isoformat()}_{now.strftime('%H%M%S')}.md"
...
def main() -> None:
    ...
    logger.info(f"✅ News Radar 完成: {report_path}")
    
    # 自动转HTML
    import subprocess
    subprocess.run(['python', 'news_md_to_html.py', report_path])
