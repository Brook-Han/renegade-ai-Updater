...
# -*- coding: utf-8 -*-
"""
🎓 Renegade AI 学术论文列表页生成器 (v5.4)
视觉风格与 Renegade AI v5.4 网页完全对齐
...
# 配置
# ═══════════════════════════════════════════════════════════════
ACADEMIC_DIR = Config.OUTPUT_DIR / "academic"
...
def score_to_bars(score: str) -> str:
    """将分数转换为 5-bar HTML。"""
    try:
        s = float(score)
    except (ValueError, TypeError):
        s = 5.0
    filled = min(5, max(1, int(s / 2) + (1 if s % 2 >= 1 else 0)))
    bars = []
    for i in range(5):
        bars.append('<span class="bar filled"></span>' if i < filled else '<span class="bar"></span>')
    return ''.join(bars)
...
def generate_day_block(date: str, entries: list[dict], top_n: int = None) -> str:
    all_cards = []
    for e in entries:
        cards = extract_cards_from_html(e["path"], "academic")
        for c in cards:
    ...
    for card in display_cards:
      ...
      <div class="radar-card-top">
        ...
        <div class="radar-score">
          {score}<span>/10</span>
          <div class="score-bar">{bars_html}</div>
        </div>
      </div>
...
<head>
  ...
  <title>Academic Archive | Renegade AI v5.4</title>
...
<div class="controls-bar">
  ...
  <button class="filter-btn" data-filter="draft" aria-pressed="false">✍ DRAFT</button>
  <div class="search-wrap">
    <input type="text" id="searchInput" placeholder="SEARCH TITLES &amp; ABSTRACTS" aria-label="Search titles and abstracts">
  </div>
  <a href="../index.html" class="list-link">← MAIN DIGEST</a>
</div>
...
<main class="main">
  ...
  <div class="empty-state" id="emptyState">
    ...
    <p>NO RESULTS · ADJUST SEARCH TERMS</p>
  </div>
  <footer>
...
# ═══════════════════════════════════════════════════════════════
def main():
    print("🎓 Renegade AI Academic Index Generator (v5.4 Visual Edition)")
