...
def collect_academic_reports() -> dict:
    ...
    for html_file in sorted(ACADEMIC_DIR.glob("academic_report_*.html")):
        ...
        # ✅ 时间戳可选，格式：academic_report_YYYY-MM-DD(_HHMMSS)?.html
...
def generate_day_block(date: str, entries: list[dict], top_n: int = None) -> str:
    ...
    for card in display_cards:
        ...
        link_html = f'<a href="{escape(link)}" target="_blank" rel="noopener">↗ PDF/原文</a>' if link and link != "#" else ""

        html += f'''
  <article class="card" data-type="academic">
    <div class="card-header">
      <div class="card-title">
        <a href="{escape(link)}" target="_blank" rel="noopener">{title}</a>
        {draft_badge}<span class="type-badge academic">📄 论文</span>
      </div>
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .card-score{font-family:var(--display);font-size:1.8rem;line-height: 1;color:var(--accent);white-space:nowrap;text-align:right}
...
<body>
  ...
  <main class="main">
    <div class="page-eyebrow">§ Academic Archive</div>
    <h1>ACADEMIC ARCHIVE</h1>
    <div class="subtitle">
      <span>🎓 全部论文条目 · 按日期排序</span>
